...
namespace D2DMapEditor
{
    class Map
    {
        ...
        public Boolean ExportMap(ref Tile[] tileLibrary, PictureBox pbMap, string path, ListBox lbLayers, ExportLayer exportLayer, ProgrammingLanguage language, ImageFormat format)
        {
            ...
            {  
                case ExportLayer.SelectedOnly:
                    ...
                    layerPath = path + "\\" + Layers[index].Name + ".xml";
            ...
            // save total map image
            SaveTotalMapImage(path, pbMap);

            return true;
        }

        public static void SaveTotalMapImage(string path, PictureBox pbMap)
        {
            Image img = pbMap.Image;
            Bitmap bmp = new Bitmap(img);

            try
            {
                bmp.Save(path + "Total.png", ImageFormat.Png);
            }
            catch (Exception exc)
            {
                MessageBox.Show(exc.ToString(), "Fail to Save Total Map Image!\n" + exc.Message, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
